...
Fresh leader-serialized follow-up evidence found only **5 cases** clean across RV + LA x musl + glibc:
...
| LA targeted `followup-la-targeted-004` | RV-clean subset above | `PASS LTP CASE 11`, `FAIL LTP CASE 1`; `sched_getscheduler02` musl TFAIL=1; timeout/ENOSYS/panic/trap 0; marker bad=0. | Four-way clean subset only: `prctl05,sethostname01,setrlimit01,signal03,signal04`. |
...
| `sched_getscheduler02` | process/sched | RV clean for musl+glibc; LA glibc clean; LA musl fails `TFAIL=1` (`sched_getscheduler(4194304)` libc variant returned success instead of ESRCH). | Do not promote; inspect libc wrapper/syscall errno path on LA/musl. |
...
- Fresh follow-up parsed summaries: `raw/followup-rv-targeted-001-summary.txt`, `raw/followup-la-targeted-004-summary.txt`.
...
1. Keep the 5 fresh four-way clean cases as tranche seeds: `prctl05,sethostname01,setrlimit01,signal03,signal04`.
2. Repair one near-clean blocker, preferably `sched_getscheduler02` LA/musl ESRCH behavior, then rerun LA targeted for only that case.
3. In parallel discovery (not promotion), repair/target `pipe2_02` helper resources and `waitpid01` musl signal wait status; both failed fresh RV and should not go to LA until RV clean.
