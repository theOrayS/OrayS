...
crate 依赖方向。最终 15 个 mutation test 证明检查会拒绝普通或 target-specific 反向依赖、
boundary unsafe、legacy caller 增长、
...
5. 24 个仍直连 `linux_raw_sys` 的 uspace 文件按何种子系统顺序迁移；建议先拆只读 number/type
   consumer，再处理 dispatcher、signal、VM 和 FD，不在一个 PR 中横跨这些 major subsystem。
6. 是否为 `UserProcess`/`AddrSpace` 建立可构造的真实测试 fixture，以覆盖跨页、只读映射写和
   partial-copy；在 fixture 出现前，不能用 fake backend 单测冒充 shell runtime 语义证据。
