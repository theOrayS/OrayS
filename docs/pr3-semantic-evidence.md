...
**支持范围政策（2026-07-22 起）**：本项目支持的目标架构仅为 RISC-V64 与
LoongArch64，CI 宿主仅为 Linux。x86_64、aarch64 及其他目标不再作为 required 或
observational 门禁；macOS job 超出支持范围，不得阻塞 readiness。`Other platforms`
与 `macOS` 两类 job 已按该政策移除，而不是改为弱化的可选检查。
...
- `Clippy (riscv64, fixed-required)`、`Clippy (loongarch64, fixed-required)`
- `Build (riscv64, fixed-required)`、`Build (loongarch64, fixed-required)`
- `Application tests (riscv64, fixed-required)`、
  `Application tests (loongarch64, fixed-required)` 保留既有 pinned-toolchain
  hard-fail 覆盖；其外部 assets 不会被当成 repository-contained semantic evidence。
- `Docs (ubuntu-24.04)` 保留 Linux 文档 hard-fail 覆盖，构建主支持目标
