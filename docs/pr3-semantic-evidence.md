...
`scripts/semantic_evidence_manifest.json` 是 case、架构适用性、runner、capability 和
required/observational policy 的唯一手写事实源。manifest 与 canonical result 都使用
schema version 1；完整生成 schema 位于
`scripts/semantic_evidence_schema.v1.json`。
...
```bash
python3 scripts/semantic_evidence.py run --help
python3 scripts/semantic_evidence.py validate-result --help
python3 scripts/render_semantic_evidence.py --help
```
...
共享 `scripts/evaluator_protocol.py` 直接解析 raw bytes。它支持 official wrapper 的
...
```bash
python3 scripts/semantic_evidence.py schema \
  --write scripts/semantic_evidence_schema.v1.json
make pr3-manifest-check
make pr3-infrastructure-tests
```
