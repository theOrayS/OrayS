...
Clean current audit: `docs/ltp-1000-long-term-plan-2026-06-01/milestone-03-stable656/combined-candidate-pool-clean30-epoll-create1-20260602T061430Z.md`.
Latest incremental report: `target/ltp-1000-milestone-03-stable656/epoll-create1-clean2-20260602T061430Z.promotion-candidates.txt`.
Current pool is **30/50** for stable656; 20 more trustworthy unique cases are required before editing `LTP_STABLE_CASES`.
...
No `LTP_STABLE_CASES` edit is justified yet. The candidate pool is 30/50 for stable656, and all remaining blocker rows retain their parser-visible `TFAIL/TBROK/TCONF/ENOSYS/timeout/panic/SIGSEGV` caveats.
...
Decision: add `epoll_create1_01` and `epoll_create1_02` to the future promotion pool. The implementation is generic `__NR_epoll_create1` support with `EPOLL_CLOEXEC` and unknown-flag `EINVAL` handling, backed by a synthetic `anon_inode:[eventpoll]` FD entry sufficient for creation/close/dup/fcntl tests. Current pool is 30/50, so no stable-list edit is made.
