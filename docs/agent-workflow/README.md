...
| `ltp-selection.md` | 选择下一批 LTP/score 测例、做 ROI 排序，或规划 blacklist full sweep | 候选选择、证据要求、优先级模型、实验性 full-sweep/blacklist 口径 |
| `ltp-promotion-and-docs.md` | 修改 `LTP_STABLE_CASES`、写 LTP 阶段文档，或记录 blacklist sweep 证据 | 推广闸门、blacklist 证据边界、报告字段、文档命名 |
...
- blacklist 只能隔离 full sweep 的运行风险；被跳过 case 不算通过，不能当 promotion 证据。
- 不碰无关生成物、镜像、日志、批量格式化。
