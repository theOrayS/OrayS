...
- `architect`：`WATCH`。确认本轮修复是通用语义修复、不是测试适配；但 raw pointer copy-in/out 架构风险、pthread mutex attr/type 简化模型、未跑 live QEMU/远程评测、以及预先存在的无关脏工作区仍需作为后续项跟踪。

Ultragoal final gate 因 `architectStatus=WATCH` 不是 clean gate；本报告不声称完全 CLEAR/APPROVE，也不隐藏 WATCH 风险。
...
3. **未运行 live QEMU/远程官方评测**
   - 本轮重点是 self-check 合规审查、静态逐例矩阵、guard 和 build/typecheck；没有宣称 RV/LA live LTP 分数。
   - `make -n all` 只证明默认 runner dry-run 仍是 stable，不是远程分数证明。
