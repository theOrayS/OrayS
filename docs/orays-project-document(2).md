...
OrayS 是一个基于 ArceOS 的实验性模块化操作系统内核分支，保留了 ArceOS 的 workspace 与 feature 组合机制，同时补充了 Linux/POSIX 用户态边界、ELF 加载、进程生命周期、文件描述符、信号、futex、mmap、socket 和 LTP runner 等能力。
...
- 底层模块：提供 HAL、内存、调度、VFS、网络和驱动框架。
- 评测用户态兼容层：由 `user` 与 `api/arceos_posix_api` 共同提供 Linux ABI、系统调用分发和 LTP 执行能力。
