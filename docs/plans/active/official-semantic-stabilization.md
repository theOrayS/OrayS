...
|---|---|---|---|---|---|---|---|---|---|
| B-SPLICE-001 | RV+LA, musl+glibc | `splice01/02/04/05/06`, `tee01/02`, `vmsplice01`, `dirtypipe`; `splice02` EBADF 后 30s timeout/TBROK | pipe/splice/FD、方向与 offset errno | `bfff16ea` 首轮 happy-path 真 guest red；`ad9d1ab5` 在 test-only clean HEAD 上将 zero-len/errno/same-backing/preservation/lifecycle/O_NONBLOCK 组合再次跑红；reciprocal 并发由可执行 host thread unit 覆盖 | 旧适配器误报 EBADF；同 backing 仅比较 fd、分次表锁造成 close/reuse TOCTOU、先消费 source 再写 destination 会丢数据，且 syscall 的 len/flags/fd/offset 优先级不符 Linux | `ae446dbf` 生产修复；`e7fe68a1` 将 canonical unit 精确计数从 24 同步为实际 33 | exact-clean `e7fe68a1` 定向 outer 1/1、inner 3/3 PASS；official delta 待测 | exact-clean `e7fe68a1` 定向 outer 1/1、inner 3/3 PASS；official delta 待测 | REVIEW_PASS |
...
B-SPLICE-001 的扩展 test-only red、通用生产修复、双架构定向 green 与第二轮独立
只读审查均已完成；第二轮结论为 `0 Blocker / 0 Major / 1 Minor`，唯一文档 Minor 在本
checkpoint 同步。下一步在精确 clean 文档候选上依次运行 quick、baseline 与 fresh
RV/LA official。official delta 仍只核对本 cluster 有明确因果预期的 splice/tee/
vmsplice identity；RV cyclictest 与其他 cluster 保持未归因，不因 pipe 相关性提前计功。
