...
|---|---|---|---|---|---|---|---|---|---|
| B-SPLICE-001 | RV+LA, musl+glibc | `splice01/02/04/05/06`, `tee01/02`, `vmsplice01`, `dirtypipe`; `splice02` EBADF 后 30s timeout/TBROK | pipe/splice/FD、方向与 offset errno | `bfff16ea`/`ad9d1ab5` 覆盖首批 splice 行为；`8e48e853` 新增非 pipe/same-pipe tee errno 与跨 64 KiB iovec 的 blocking vmsplice partial-progress/全字节顺序/清理；RV 在该 test-only commit 真实 `USER_FAIL`，`44a25cff` 后真实 guest watchdog timeout | 已证实两个独立通用缺陷：有效非 pipe fd 被 tee helper 误报 EBADF；vmsplice 在已有进展且 pipe 恰于 iovec 边界填满后仍阻塞处理下一 vector | 首批 `ae446dbf`；canonical count `e7fe68a1`；第二批测试 `8e48e853`、tee errno `44a25cff`、vmsplice progress `64af8ac4` | 上轮 official 114→87；新修复在 exact-clean `64af8ac4` RV targeted outer 1/1、inner 3/3 PASS，新 official delta 待 review/quick/baseline 后运行 | 上轮 official 157→127；exact-clean `64af8ac4` LA targeted outer 1/1、inner 3/3 PASS，新 official delta 待运行 | TARGETED_GREEN_REVIEW_PENDING |
...
- [x] `8e48e853` 在同一 freestanding guest 中增加通用 tee errno 与跨 iovec vmsplice
  进展/数据/清理回归；exact-clean RV 的 tee red 为 guest nonzero，`44a25cff` 后的
  vmsplice red 为 10 秒 guest watchdog，均是 build 2 PASS / runtime 1 ERROR。
- [x] `44a25cff` 区分 tee 的 live non-pipe `EINVAL` 与 invalid/wrong-direction `EBADF`；
  `64af8ac4` 使 blocking vmsplice 在已有累计进展后以非阻塞容量探测返回 partial count，
  不按 case/libc/arch/path 特化。
- [x] exact-clean `64af8ac4` 的 RV/LA `evidence-runtime` 外层各 1/1 PASS、内层各
  3/3 PASS；marker 唯一有序，cleanup/reap 完整，无 FAIL/panic/timeout/残留进程。
- [ ] 完成本批独立只读 review；0 Blocker / 0 Major 后才运行 clean quick/baseline。
...
第二批 B-SPLICE-001 已在 test-only `8e48e853` 保存 tee red，并在仅含 tee 修复的
`44a25cff` 保存 vmsplice guest-timeout red；`64af8ac4` 上 RV/LA targeted 均明确 green。
下一步由未参与实现的只读 reviewer 检查 `4529a255..64af8ac4` 的 ABI/errno、partial
progress、用户 iovec、资源清理、双架构与测试真实性。只有 review 为 0 Blocker /
0 Major 后才运行 clean quick、baseline 和 fresh 双架构 official delta。RV 的
`epoll_wait02`/`nanosleep01` 与 LA musl `kill02` 继续作为真实待归因 identity，首次
FAIL 不被重试覆盖。
