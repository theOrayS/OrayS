...
|---|---|---|---|---|---|---|---|---|---|
| B-SPLICE-001 | RV+LA, musl+glibc | `splice01/02/04/05/06`, `tee01/02`, `vmsplice01`, `dirtypipe`; `splice02` EBADF 后 30s timeout/TBROK | pipe/splice/FD、方向与 offset errno | 首轮 `pipe2 -> write -> splice -> read` 在 `bfff16ea` 上真实 guest red；errno/self-splice/full-destination/lifecycle/concurrency 补测待先红 | 首轮已证明旧适配器对迁移后的 Pipe 误报 EBADF；只读复核进一步发现同 backing 仅比较 fd、两次表锁间 close/reuse TOCTOU、pipe-to-pipe 先消费后写入会丢数据，以及 `len == 0`/fd/offset 错误优先级不符 Linux | `b9d90a15` 仅为首轮部分修复 | 首轮 inner 3/3 PASS；补测与 official delta 待测 | 首轮 inner 3/3 PASS；补测与 official delta 待测 | REVIEW_MAJOR |
...
- [ ] 先红补齐 `len == 0`、无效 fd、pipe offset、错误方向、同 backing pipe、目标已满且
  source 数据保留、close/reuse 生命周期和 reciprocal 并发锁序行为。
- [ ] 生产实现按 pipe 对象身份判 self-splice，在同一有序双锁临界区迁移数据，并在取用
  endpoint 时固定 `OpenFileRef` 生命周期；修正 syscall 入口的 len/fd/offset 优先级。
- [ ] 修订后的 RV/LA 定向行为测试明确 PASS，且首次失败证据完整保留。
- [ ] 独立只读 cluster review 为 0 Blocker / 0 Major。
- [ ] clean quick 与 baseline 明确 PASS。
- [ ] fresh RV/LA official identity delta 与预期一致，且无新增 failure identity。
...
首轮独立只读审查以 `0 Blocker / 3 Major` 拒绝放行。下一步先扩展通用行为回归并在
`b9d90a15` 基线上保存真实 red，再实现同 backing 判定、pipe-to-pipe 原子搬运、稳定
endpoint 生命周期和 Linux 入口错误优先级；完成双架构定向 green 与复审清零后，才在
exact clean 候选上依次运行 quick、baseline 与 fresh RV/LA official。official delta 仍只
核对本 cluster 有明确因果预期的 splice/tee/vmsplice identity；RV cyclictest 与其他
cluster 保持未归因，不因 pipe 相关性提前计功。
