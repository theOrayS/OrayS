...
状态：`PAUSED_BY_EXPLICIT_USER_INSTRUCTION`（非终态）
...
运行时证据截止：`789f9a3dd725185a85c85c16502bf86d53dd873c`
...
|---|---|---|---|---|---|---|---|---|---|
| B-SPLICE-001 | RV+LA, musl+glibc | `splice01/02/04/05/06`, `tee01/02`, `vmsplice01`, `dirtypipe`; `splice02` EBADF 后 30s timeout/TBROK | pipe/splice/FD、方向与 offset errno | `bfff16ea`/`ad9d1ab5` 覆盖首批 splice 行为；`8e48e853` 引入跨 iovec vmsplice 测试但该 run 在 tee 先失败，`44a25cff` 才首次真实触发 vmsplice watchdog；`5c365cc6` 建立修订顺序 red；`324e3f4c`/`789f9a3d` 用真实 `/dev/null` mode 建立 device red | 已证实三个相关通用缺陷：tee 缺少 zero-length fast path并过早分类对象类型；`DevNull`/`Rtc` 创建时丢弃 open access mode，令 tee 无法先判 `EBADF`；vmsplice 在已有进展且 pipe 恰于 iovec 边界填满后仍阻塞处理下一 vector | 首批 `ae446dbf`；canonical count `e7fe68a1`；vmsplice progress `64af8ac4`；修订 tee 顺序 `5c365cc6`/`d75a4d8e`；device-mode 测试 `324e3f4c`/marker `789f9a3d`，生产修复待提交 | 上轮 official 114→87；exact-clean `789f9a3d` RV device-mode outer 1/1 FAIL、inner 2 PASS/1 ERROR，生产修复后须重跑 | 上轮 official 157→127；`d75a4d8e` LA targeted 1/1 PASS 不能覆盖 device-mode Major，修复后须重跑 | TARGETED_RED |
...
- [x] `8e48e853` 引入跨 iovec vmsplice 测试，但其 run 在 tee 断言处先退出，不能计为
  vmsplice runtime red；`44a25cff` 越过 tee 后的 guest watchdog 才是真实 vmsplice red。
  `64af8ac4` 修复 partial progress，且 exact-clean RV/LA targeted 均 green。
...
- [x] 本批二次独立只读 review 为 `0 Blocker / 1 Major / 1 Minor / 0 Nit`，继续拒绝
  promotion。Major：`DevNull`/`Rtc` 丢弃 open access mode，`tee_fd_snapshot` 硬编码
  双向可用，使错误方向返回 `EINVAL` 而非 `EBADF`。Minor：上述 vmsplice red 归属不精确。
- [x] `324e3f4c` 增加真实 `/dev/null` O_RDONLY/O_WRONLY 方向矩阵；首次 RV red 保留，
  `789f9a3d` 仅细分失败 marker 后的新 run 明确出现 `USER_FAIL tee_device_mode`。
  exact-clean outer 1/1 FAIL、inner 2 PASS/1 ERROR，无 timeout/crash/infra，cleanup/reap 完整。
- [x] 2026-07-18 按用户明确指令停止生产修复与新 official/full，仅用既有可核验证据建立
  `docs/defect-and-reliability-status-2026-07-18.md` 防御性缺陷/可靠性快照；该动作
  不改变 runtime 状态、failure identity 或 Goal B 门禁结论。
- [ ] 持久保存 `DevNull`/`Rtc` mode，补齐构造、fork/dup、snapshot guard/mutation 和
  双架构 green。
- [ ] 第三次独立只读 review 达到 0 Blocker / 0 Major 后才运行 clean quick/baseline。
...
第二次独立 review 已确认 `d75a4d8e` 的 flags/zero/fd/access/type 主顺序正确，但发现
`DevNull`/`Rtc` 创建时丢弃 open access mode 的 Major，结论仍为 0B/1M，promotion 封锁。
`324e3f4c` 已提交只含 `/dev/null` O_RDONLY/O_WRONLY tee 方向矩阵的测试；首次 RV red
完整保留。`789f9a3d` 只细分 marker 后，第二个 exact-clean RV run 明确在
`USER_FAIL tee_device_mode` fail closed。

2026-07-18 用户明确要求不执行 Goal resume、不继续 Goal B 生产修复、不启动新的
official/full。本计划因此停在非终态 `PAUSED_BY_EXPLICIT_USER_INSTRUCTION`；本次只完成
独立文档收尾、Git 核对与获授权的普通分支推送。未来只有收到新的明确恢复指令后，才以
通用 mode 表示修复 `DevNull`/`Rtc`，绑定构造、fork/dup 与 snapshot 的 guard/mutation，
完成 RV/LA targeted green 后第三次只读复审。只有复审达到 0B/0M 才运行 clean
quick/baseline；其后 fresh official delta 仍必须让 `tee02`、`vmsplice01` 消失且无新增
identity。RV 的 `epoll_wait02`/`nanosleep01` 与 LA musl `kill02` 继续作为真实待归因
identity，首次 FAIL 不被重试覆盖。
