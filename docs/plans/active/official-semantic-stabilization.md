...
|---|---|---|---|---|---|---|---|---|---|
| B-SPLICE-001 | RV+LA, musl+glibc | `splice01/02/04/05/06`, `tee01/02`, `vmsplice01`, `dirtypipe`; `splice02` EBADF 后 30s timeout/TBROK | pipe/splice/FD、方向与 offset errno | `bfff16ea`/`ad9d1ab5` 覆盖首批 splice 行为；`8e48e853` 引入跨 iovec vmsplice 测试但该 run 在 tee 先失败，`44a25cff` 才首次真实触发 vmsplice watchdog；`5c365cc6` 按 flags→zero-length→输入 fd→输出 fd→access mode→pipe type 建立修订 tee red | 已证实三个相关通用缺陷：tee 缺少 zero-length fast path并过早分类对象类型；`DevNull`/`Rtc` 创建时丢弃 open access mode，令 tee 无法先判 `EBADF`；vmsplice 在已有进展且 pipe 恰于 iovec 边界填满后仍阻塞处理下一 vector | 首批 `ae446dbf`；canonical count `e7fe68a1`；vmsplice progress `64af8ac4`；修订 tee 测试 `5c365cc6`、顺序实现 `d75a4d8e`（device mode Major 待修） | 上轮 official 114→87；exact-clean `d75a4d8e` RV targeted 1/1 PASS 但二审发现 device-mode Major，须新 red/fix/green 后重审 | 上轮 official 157→127；`d75a4d8e` LA targeted 1/1 PASS 同样不能覆盖 device-mode Major | TARGETED_FIX_REQUIRED |
...
- [ ] 先以真实 `/dev/null` O_RDONLY/O_WRONLY 描述符建立 test-only red，再持久保存
  `DevNull`/`Rtc` mode，补齐构造、fork/dup、snapshot guard/mutation 和双架构 green。
...
下一步先提交只含 `/dev/null` O_RDONLY/O_WRONLY tee 方向矩阵的测试并保存 exact-clean
RV red，再以通用 mode 表示修复 `DevNull`/`Rtc`，绑定构造、fork/dup 与 snapshot 的
guard/mutation，完成 RV/LA targeted green 后第三次只读复审。只有复审达到 0B/0M 才
运行 clean quick/baseline；其后 fresh official delta 仍必须让 `tee02`、`vmsplice01`
消失且无新增 identity。RV 的 `epoll_wait02`/`nanosleep01` 与 LA musl `kill02` 继续作为
真实待归因 identity，首次 FAIL 不被重试覆盖。
