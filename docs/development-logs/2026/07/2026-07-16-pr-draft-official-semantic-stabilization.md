...
- B-SPLICE-001 首批扩展先红、修订实现、复审、clean quick/baseline 与 fresh official
  ...
  device-mode red/fix/双架构 green、第三次 review、clean quick/baseline 与 fresh
  official delta 均尚未完成，不能把 targeted green 计为 cluster PASS。
...
先增加真实 `/dev/null` O_RDONLY/O_WRONLY tee access-mode test-only red，再让
