...
head_commit: "64af8ac466aa0ca1a96e8373e51d1bf4aebe57d5"
...
## 2026-07-17 — Checkpoint 9：tee errno、vmsplice partial progress 先红与双架构 green
...
- test-only 提交 `8e48e853` 在既有 freestanding smoke 中覆盖三条通用 tee 合同：live
  console/pipe 两种非 pipe 组合与同 backing pipe 均为 `EINVAL`；又用 64 KiB vector
...
- `44a25cff` 把 tee-only helper 的 live non-pipe 结果改为 `EINVAL`；无效 fd 仍由 fd table
  返回 `EBADF`，pipe 错误方向仍由 endpoint 返回 `EBADF`。静态 guard 和 mutation 将
  三者边界绑定。该 exact-clean commit 的 RV run 越过 tee 后由 10 秒 guest watchdog
...
- B-SPLICE-001 首批扩展先红、修订实现、复审、clean quick/baseline 与 fresh official
  delta 已完成；14 个目标 identity/架构消失。第二批 `tee`/`vmsplice` 已完成通用
  test-only red、两个生产修复和双架构 targeted green，但独立 review、clean quick/
  baseline 与 fresh official delta 尚未运行，不能把 targeted green 计为 cluster PASS。
