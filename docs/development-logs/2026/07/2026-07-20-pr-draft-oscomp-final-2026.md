...
不能从后续候选或历史记录倒推 baseline。仓内继续实现或证明 relative `FUTEX_WAIT` 的
remaining-time restart block，并审计 futex requeue、signal/timeout 与比较-排队竞争；若状态
生命周期不能闭合则保持显式 `EINTR`，不以延长 timeout 的 frame replay 冒充语义完成。
