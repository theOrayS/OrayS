...
## 2026-07-21 — Checkpoint 39：`FUTEX_CMP_REQUEUE` 目标验证顺序 RED -> GREEN

- host Linux 原始 syscall 对照把 source 置为 1、expected 置为 0：非对齐 target 返回
  `EINVAL`，null target 返回 `EFAULT`，有效 target 才返回 `EAGAIN`。Linux 当前
  `b95f03f04d475aa6719d15a636ddf32222d55657` 的 `futex_requeue()` 也先取得 source/target
  key，再在 bucket locks 下读取比较值。OrayS 旧 helper 则把 `uaddr2 == 0` 提前映射为
  `EINVAL`，并在 `futex_key(uaddr2)` 前读取/比较 source；因此 null errno 错误，已解除映射的
  对齐 target 又会被 source mismatch 短路为 `EAGAIN`。
- test-only `ebc0de864c0271fecf98115abb6cb45ad12327d1` 增加
  `futex_cmp_requeue_validation`，同一次真实 syscall 合同分别要求 null/unmapped target 为
  `EFAULT`、misaligned target 为 `EINVAL`、有效 target mismatch 为 `EAGAIN`。测试中新
  `unsafe` 仅用于 freestanding 原始 syscall，注释明确了有效指针生命周期和三个故意非法
  kernel-boundary 输入。修复前 RV/LA 都先通过前两个 requeue marker，随后明确
  `USER_FAIL futex_cmp_requeue_validation`、`HARNESS_FAIL guest_nonzero_exit` 与正常关机，无
  panic、trap 或 timeout；RED log SHA-256 为
  `e370932e1fea54bfe337cbcdb9b24f895c6051d793b8e1e73f25437c1bc54536`、
  `b347e120208ff2ad36858b9f727dda1188a12729b63deed95cfeb2c28a006c54`。
- `e71c46cfcebfa4d085bd643506bbe68d554b5be7` 删除 null-as-`EINVAL` 特判，保留四字节对齐
  `EINVAL`，随后先解析 source/target key，再执行可选 source comparison。它没有改变无 waiter
  返回、wake/requeue 数量、private/shared key、ABI 或架构路径。静态 guard 固化 null EFAULT
  路径与 source-key < target-key < compare 顺序，两项 mutation 都能被检出；focused guard
  为 0 finding，mutation 29/29、semantic 75/75 PASS，RV/LA smoke build 均 exit 0、warning
  保持 175。两架构 dirty GREEN 都有新 marker、`USER_PASS`、`HARNESS_PASS`、`SHUTDOWN`，
  log SHA-256 为
  `18997e1ed1cf3bed5a866ca26413491a334912e508221306d5a45104a537b3a9`、
  `6a3520802cdffe4bb03fdc00d9336095242ebcf9e0ff46cb976c277b94387423`。
- 两条误用的 `make pr3-smoke-build-{rv,la}` 在构建前均 exit 2，原因是目标不存在；随后从
  Makefile 解析并使用 `pr3-smoke-kernel-{rv,la}` 成功重跑，不把前者计作测试。注册提交
  `07ff19817aca4b137a72978427950983ea409a2d` 将 canonical/manifest pin 从 27 收紧到 29；
  29/29 mutation、36/36 asset、135/135 runner regression 和完整 suite list 均 PASS。
- clean `07ff1981` canonical RV evidence-runtime 为 1/1 PASS、零非 PASS，177.742963 s；
  summary/inner/console SHA-256 为
  `223ad0ff0f4a016e0928764323ec963c247935a28b3450ce393ad7d7d321f744`/
  `c681ea7fe16b2192f969e8034fcb0aff2a561be1842e20beca0282ace42f1832`/
  `108e55adf508a266a4c71d0b4e3ee8d4eb8aa57f8bf3a6d9a30ae1ccb6957ba6`，证据在
  `test/output/20260721T145920Z-evidence-runtime-rv-3728164/`。同轮并行 LA 首次执行明确
  FAIL：inner console 在 `futex_sa_restart` 后被 supervisor 标记 `guest_timeout`，outer
  summary 为 1 FAIL、175.754790 s、SHA-256
  `47c64695368e0574c692fd747966effac3f9a3fa6a7a1ac6caaa58702666ea8e`，证据在
  `test/output/20260721T145920Z-evidence-runtime-la-3728165/`；该结果保留，未以重试覆盖。
  停止并行争用后，LA 顺序诊断重跑 1/1 PASS、零非 PASS，102.656358 s；summary/inner/
  console SHA-256 为
  `1a7597af78e8999ba7e5968268e4d11b635f0b9fc544fc1e36840c7ec6bb6238`/
  `644543c448eadfe3798767f81b3dafe6b754bf805a6985c59387d07f6eacc938`/
  `8ca90cd375a6a08205220cbd3b37fb8f5d7b10220130017540c9c271db57a8fa`，证据在
  `test/output/20260721T150247Z-evidence-runtime-la-3730890/`。三次 outer provenance 均
  clean/stable；两个最终 inner evidence required 3/3 PASS 并绑定 `07ff1981...`。
- 同一 clean commit canonical quick 为 47/47 PASS，零 FAIL/TIMEOUT/CRASH/INFRA/NOT_RUN，
  340.548009 s；summary SHA-256
  `ec412d1a3584dd16d81a70f9a42626c2110bad0092585750d4a22580565bb4c7`，证据在
  `test/output/20260721T150445Z-quick-none-3732224/`。本 checkpoint 只闭合确定性地址验证与
  errno 优先级；OrayS 的 source compare 仍发生在实际 queue locks 之前，尚未满足 Linux
  compare/requeue 原子区合同，也未用不稳定竞态测试冒充解决。正式 final rootfs、合格
  8c/8G host 与官方四格得分继续 `BLOCKED`/未执行。

## 2026-07-21 — Checkpoint 40：`FUTEX_CMP_REQUEUE` queue-lock 原子比较

- Linux 当前 `b95f03f04d475aa6719d15a636ddf32222d55657` 的 `futex_requeue()` 在取得
  source/target bucket locks 后才用 `futex_get_value_locked()` 比较 source，比较成功后在同一
  锁域直接 wake/requeue。Checkpoint 39 后的 OrayS 虽已先验证两个 key，却仍在进入任何
  WaitQueue lock 前读取/比较 source；waiter enqueue、wake 或 requeue 可以穿越该窗口。这是
  源码可证明的原子性缺口，但竞态命中率不稳定，因此没有用概率型 guest 运行冒充确定性 RED；
  既有真实 CMP validation runtime 与 scoped source mutation 共同约束本次结构修复。
- `c59e25779c4c1fc64e53532052257e0c528ae0e7` 为 `WaitQueue` 增加通用
  `notify_and_requeue_where_checked`：沿既有同对象单锁或不同对象稳定地址锁序取得所有相关
  queue locks，再在选择、唤醒或移动 waiter 前执行 fallible `check`；失败时两个队列均保持
  不变。原 `notify_and_requeue_where` 通过 `Infallible` check 包装该实现，普通调用语义不变。
  futex 层为 CMP 路径取得 source/target state，即使当前无 waiter 也进入相同 queue-lock 路径，
  在 check closure 内读取并比较 source，只在验证成功后递增 source sequence。该锁序与既有
  wait condition 的“先 queue lock、再读用户值”一致；`futex_key` 仍在加锁前预检用户页。
- axtask 新增单元测试验证 checked error 确实执行且 source/target 队列为空。静态 guard 约束
  queue lock 后、任一 mutation 前 `check()?`，并约束 futex comparison 位于 checked call 内；
  mutation 覆盖把比较移回锁外、退回 unchecked path、删除 validation。第一轮 focused unit
  真实出现 2 项 FAIL：guard parser 不能识别缩进的 impl method，旧“compare before target key”
  fixture 又不再匹配新结构；修正 parser/fixture 后仍有 1 项 FAIL，原因是 rustfmt 后
  `check()?` 缩进与 mutation replacement 不符。三处均修正后 boundary guard 0 finding、
  mutation 32/32、no-fake-success guard 0 finding、semantic evidence 75/75 PASS；
  `cargo test -p axtask --features sched-fifo,test` 为 5/5 unit 与 2/2 doc PASS，仅保留既有
  `unblock_task` dead-code warning。RV/LA smoke kernel build 均 exit 0、各 175 warnings。
- dirty sequential GREEN 两架构都看到完整 semantic marker、`USER_PASS`、`HARNESS_PASS`、
  `SHUTDOWN`，无 timeout/deadlock；RV/LA log SHA-256 分别为
  `663cd882dbe334f5d8ed168e55c0cbbf040975cddafedf9a9b4dd5890cef3b2b` 与
  `c5000a97b6310f4e435dfb4333fe9f69161c24869e3f8cde60c0f79cff051eec`，只作为定向诊断。
  注册提交 `7aa946391fbfec4a0cac9869727dca46d1570116` 将 runner/manifest pin 从 29 收紧到
  32；32/32 mutation、36/36 asset、完整 suite list 与 135/135 runner regression 均 PASS。
- clean `7aa94639` sequential canonical RV/LA evidence-runtime 各 1/1 PASS、零非 PASS，耗时
  104.896086/101.688061 s；summary SHA-256 为
  `fe29687466f0dc40f31aad18fa4ed8e2281470e7a9d55509c2305f8b1ad5e9b7`/
  `accfd3534f1e14c1072146be2f1a00de3ff9435969c54e01d71ea64d25ffc488`，inner evidence 为
  `d2fb97fcccef597f88512293e252555e6049bfdbffef906e30e1f839dea0db16`/
  `54baac4c6ed74503b3d87c65a16d9ee65ffd55db40a1172ac2bdf207d33155a0`，console 为
  `b835040a799ff584801607d3cefb545c502fc54aabe1ddc704e57e861b08f3a4`/
  `b8c35555cb6649f482b5516102f645ea03d502a41be3a6535445ef17f7cbd9af`；证据目录为
  `test/output/20260721T153003Z-evidence-runtime-rv-3746278/`、
  `test/output/20260721T153154Z-evidence-runtime-la-3747643/`。outer/inner provenance 均
  clean/stable，inner required 3/3 PASS 并绑定精确 revision 和内容哈希
  `fdf9f5c39bbbbe2dcc61c76c7d00482d7daf1bfca047edbdb02a9e994cf606df`。
  同 commit quick 为 47/47 PASS、零非 PASS，340.000249 s，summary SHA-256
  `f916614d441bbc9ba6256f2693ff15fc84047dbceddaa1245d93d3c019bdc187`，证据在
  `test/output/20260721T153341Z-quick-none-3748971/`，并执行精确 32 项 syscall mutation。
  该 checkpoint 只闭合 WaitQueue 层可审计的 compare/wake/requeue 锁域，不宣称多 waiter、
  地址并发 unmap/remap、signal/timeout 压力或正式 CAgent/BuildStorm 得分；final 资源缺口不变。

## 2026-07-21 — Checkpoint 41：futex source NULL errno RED -> GREEN

- x86_64 host Linux 原始 `SYS_futex` 对照得到：`FUTEX_WAKE(NULL)` 与 `FUTEX_WAIT(NULL)` 均
  `-1/EFAULT`，live word + 1 的非对齐 wake 为 `-1/EINVAL`，有效无 waiter wake 返回 0。
  Linux 固定 commit `b95f03f04d475aa6719d15a636ddf32222d55657` 的 `kernel/futex/core.c`
  `get_futex_key()` 同样先按 futex size 检查自然对齐并返回 `EINVAL`，再用 `access_ok` 对不可访问
  地址返回 `EFAULT`。OrayS 旧 `sys_futex` 入口却把 `uaddr == 0` 与非对齐合并为 `EINVAL`，使
  WAIT/WAKE 的合法错误分类偏离 Linux。
- test-only `28a4b3a7b1fd70809a722eaee27c0c5e653349d8` 增加路径与程序名无关的
  `futex_source_validation`：真实调用 NULL WAIT、NULL WAKE、misaligned wake 与 valid wake，
  分别要求 `EFAULT/EFAULT/EINVAL/0`。freestanding 测试的 `unsafe` 仅限原始 syscall；live
  atomic 生命周期有效，NULL 与 interior misalignment 是故意的 kernel-boundary 输入且不在
  用户态解引用。生产代码未改时，clean RV/LA canonical 都先通过此前全部 futex marker，再明确
  `USER_FAIL futex_source_validation`、`HARNESS_FAIL guest_nonzero_exit` 与 `SHUTDOWN`，无
  timeout/panic/trap。RV/LA RED duration 为 104.810349/102.023393 s，summary SHA-256 为
  `423e69b55d123161e073ba794b956dba87aaf89d5cb767cbd53904cbb44bbe7d`/
  `ba626f365854b8e92c023ac50c2d8872c3659fafa38247a4b59a3adff642dc09`，console 为
  `400d09bd83d67f945a869372dfdbb395ecff8e5f8ad9040b1f752094de024487`/
  `4a0c79877ed0528356f446212fb10aa3468cf29d408638ae7699a8cfe984f71d`，证据目录为
  `test/output/20260721T154802Z-evidence-runtime-rv-3756333/`、
  `test/output/20260721T155000Z-evidence-runtime-la-3757931/`。
- `d1652414f6db29a52f3192fcebff0d7b3fcf2092` 仅删除入口的 NULL-as-`EINVAL` 条件：四字节
  非对齐仍立即 `EINVAL`，已实现命令随后通过既有 `fault_in_user_read`、`futex_key` 或
  `read_user_value` 把不可访问地址映射为 `EFAULT`。静态 guard 要求 alignment-only 条件并拒绝
  NULL 特判，mutation 精确恢复旧条件；focused guard 0 finding、33/33 mutation、
  no-fake-success guard 0 finding、semantic 75/75，RV/LA smoke build 均 exit 0、各 175
  warnings。dirty sequential GREEN 两架构都有新 marker、`USER_PASS`、`HARNESS_PASS`、
  `SHUTDOWN`，log SHA-256 为
  `8617acd51bb5acca709a4830e53ed7757bc9d9f68f48cfd9ec8077097fc177c0`/
  `3ca8534b0d7e2bd0ae9c7356ced5457f7a3a4e88e6744b5c0b2710648ccaa1c8`。
- 首次直接运行 raw RV target 未提供 supervisor 环境，Makefile 在 QEMU 前按设计 exit 2 并提示
  `refusing unsupervised QEMU`；随后使用 canonical runner 获得上列 RED。注册阶段又误写不存在的
  `test/checks/check_test_assets.py`，Python 在执行测试前 exit 2；改用真实
  `check_test_asset_integrity.py` 后 PASS。这两个命令错误均未计作测试结果。注册提交
  `ed4cba1db247997d24a5277a4e3c52f83adcabc9` 将 manifest/runner pin 从 32 收紧到 33；
  33/33 mutation、36/36 asset、完整 suite list 与 135/135 runner regression 均 PASS。
- clean `ed4cba1d` sequential canonical RV/LA evidence-runtime 各 1/1 PASS、零非 PASS，耗时
  105.908556/102.082055 s；summary SHA-256 为
  `028941d52a2bb0cc8b3b87fe061156e68394ed7fd6fe86647a6aca2bc10140ff`/
  `8c9fc209ca3c9e81bb137672025d686dd0d4f6eb658c4766e6fad6e5749c8220`，inner evidence 为
  `02b95ecd6ec08fa78389c59860bd5edf72b787e3bfd4b0104472e3a96cd80ba1`/
  `7755846f3c0d7f1307bb1c5ddd0406b070447f12e789c51c6b8a2e323f6200d9`，console 为
  `b7223dfbee883a394e61eabaeea6b2b0cead9f376caee41cbf188944364ee387`/
  `e0195ce7b1f2c17a362244f678dd62b974ba5622cb8c4539cf170d0c915e40e6`；证据目录为
  `test/output/20260721T160029Z-evidence-runtime-rv-3765327/`、
  `test/output/20260721T160251Z-evidence-runtime-la-3766894/`。outer/inner provenance 均
  clean/stable，inner required 3/3 PASS，revision 为 `ed4cba1d...`、共同 content SHA-256 为
  `8b6605b31b8e60e3d4eb8db478519074692d6a5b7d4ce4698a98ca418cf8139c`。
  同 commit quick 为 47/47 PASS、零非 PASS，344.357334 s，summary SHA-256
  `69004df301c67305bd401be8bfc82ba2f30af72ef2cb2232ad722ae2920c08ed`，证据在
  `test/output/20260721T160450Z-quick-none-3768265/`；syscall mutation 精确执行 33 项。
  以上只证明通用 futex source errno 与仓内回归，不是正式 CAgent/BuildStorm 执行或分数。

## 2026-07-21 — Checkpoint 42：futex operation validation `ENOSYS` RED -> GREEN

- x86_64 host Linux 原始 syscall 对照确认：`FUTEX_WAIT`、`FUTEX_WAKE`、`FUTEX_REQUEUE`
  携带 `FUTEX_CLOCK_REALTIME`，未知 flag `0x400`，以及未实现 command 2 都返回 `ENOSYS`；
  `FUTEX_WAIT_BITSET|FUTEX_CLOCK_REALTIME` 携带已过期的绝对 timeout 则正常返回
  `ETIMEDOUT`。把 source 换为 NULL 或非对齐地址后，非法 operation 仍优先返回 `ENOSYS`。
  Linux 固定 commit `b95f03f04d475aa6719d15a636ddf32222d55657` 的
  `kernel/futex/syscalls.c::do_futex()` 同样先拒绝不支持的 realtime command，再由 default
  返回 `ENOSYS`。OrayS 旧 dispatcher 只 mask command 后进入 match，未知 flag 被忽略、未知
  command 返回 `EINVAL`，且 source alignment 检查错误地先于 operation validation。
- test-only `fb4028edd6ab52130afecc751abec1b3cf312959` 增加
  `futex_operation_validation`：真实覆盖 WAIT/WAKE/REQUEUE realtime、未知 flag、未实现
  command 与受支持的 realtime WAIT_BITSET 控制。生产代码未改时，clean RV/LA canonical
  都先通过此前全部 futex marker，再明确 `USER_FAIL futex_operation_validation`、
  `HARNESS_FAIL guest_nonzero_exit` 与正常 `SHUTDOWN`；无 timeout、panic、trap 或基础设施错误。
  RV/LA summary SHA-256 分别为 `5b1250eee7b4589a3092d73c038f1568a6db548b2ac13ca0e5c0836ab818cf27`
  与 `b21f3fe93354178ea198203aff8e2812a1db72a3f088418c86209cb8ef5eef5e`，console 分别为
  `f88d8245a38bf1e6d80928370453f372639c519340831818f6fa3b8934ef1b90` 与
  `e2b3359c2faf269e6e364250f14191a937b2e40b6548fd38ab26f587daf44495`；该 RED 不计 PASS。
- `f45334cc2d30e4d5ff4c8bb71d2356ad4af9cd02` 在 `sys_futex` 入口统一解码 command、已知
  flags 与 clock 组合：未知 flag/command，以及除 WAIT_BITSET 外当前命令携带 realtime flag
  均在 source address validation 前返回 `ENOSYS`；六个现有受支持 command 继续进入原有通用
  路径，WAIT_BITSET realtime 继续使用绝对 timeout。实现不按测试程序、地址值、路径或架构
  特化，没有新增 production `unsafe`、依赖、syscall 或 ABI。三项 scoped mutation 分别约束
  unknown flag、WAIT realtime 与 operation-before-address 顺序，focused guard 36/36、semantic
  unit 75/75、RV/LA build 均 PASS；dirty GREEN console SHA-256 为
  `63af86070336464a773ed68e8f2c00bceba8077b1c2651421889ace76e94fa39` 与
  `99edb62ad1d416ed2e537c822552610b45fca5229689831059e5d6685ed341bd`。
- 注册时首次误把 `unit.competition_semantic_evidence` 的固定计数从 33 改成 36，完整 suite
  list 在执行任何 case 前以 `observed=33, declared=36, identity_binding=True` 的
  infrastructure error 明确关闭；没有计 PASS。修正为 competition 33、syscall 36 后，
  `677bfbcfa82562242af70bf1b648bc5d50a61d2b` 完成 versioned pin，mutation 36/36、asset
  36/36、完整 list 与 runner 135/135 均 PASS。
- clean `677bfbcf` 上 RV/LA canonical runtime 各 1/1 PASS，零 FAIL/TIMEOUT/CRASH/INFRA，
  outer/inner provenance clean、stable，required 3/3 PASS，并明确输出
  `ASSERT futex_operation_validation PASS`。RV summary/inner/console SHA-256 分别为
  `73cb1d64ddb37ed268be7b855b0319817fd502f135350340ac18a921351bccb6` /
  `51653d52389b41d184d0b1d921b38d88a5bb1e14bdf0d2683c37c19ad0f3440c` /
  `349df71971700e5afc798685349ee69cb2aeef0a1c9ebcccd6c6aed98c43acee`；LA 分别为
  `dd41b683859f46477a7c9094ec55e8268f2d146d60b4c03a2758981e6f7d9ab6` /
  `1740dfa1a486e866335bbfb1ef68c8681bbac6b4ab568cfccb757e01fa382ebb` /
  `ffd9eb5562757b89bcfbddc67decc32be048b81adb85fb34bf15b9f88ca8b630`。
  同一 commit 的 canonical quick 47/47 PASS，精确执行 36 项 syscall mutation，summary
  SHA-256 `c9dd158ec76503be4155a4924fc9cb0c2b6183b8f8a12ed6afcaf66a973fbf59`。
- 本 checkpoint 只证明已覆盖 operation/flag 组合及其高于 source address 的 errno 优先级；
  Linux 顶层 syscall 会在 `do_futex()` 前 copy-in 非空 timeout，而 OrayS 当前 timeout 读取顺序
  仍可能被 operation 或 source compare/address validation 遮蔽。这是下一独立审计项，未在此
  宣称闭合；也没有正式 final rootfs CAgent/BuildStorm 运行或分数。

## 2026-07-21 — Checkpoint 43：futex timeout validation order RED -> GREEN

- x86_64 host 原始 syscall 矩阵确认：mismatch futex word 携带坏 timeout pointer 返回
  `EFAULT`、非法 `tv_nsec` 返回 `EINVAL`，而空或有效 timeout 才返回 `EAGAIN`；坏 timeout
  pointer 同样优先于 source misalignment、`WAIT|CLOCK_REALTIME` 的 `ENOSYS`，以及
  WAIT_BITSET 的 word/zero-bitset 检查。未知 command/flag 不属于 timed command，仍直接
  `ENOSYS`。`LOCK_PI`、`LOCK_PI2`、`WAIT_REQUEUE_PI` 也在 operation-specific dispatch 前
  copy-in timeout。Linux 固定 commit `b95f03f04d475aa6719d15a636ddf32222d55657` 的
  `kernel/futex/syscalls.c` 以 `futex_cmd_has_timeout()` 识别五类命令，由
  `futex_init_timeout()` 校验 timespec，再调用 `do_futex()`；与 host 结果一致。
- test-only `ed46bf176322c25d3beaa621d196e1e7808c22c7` 增加
  `futex_timeout_validation`，覆盖 WAIT mismatch 的坏 pointer/非法 nsec、misaligned source、
  realtime WAIT、WAIT_BITSET/zero-bitset、三类 PI timed command 与未知 command 控制。生产
  代码未改时 clean RV/LA canonical 都先通过 source/operation marker，再明确
  `USER_FAIL futex_timeout_validation`、`HARNESS_FAIL guest_nonzero_exit` 与正常 `SHUTDOWN`；
  无 timeout、panic、trap 或基础设施错误。RV/LA duration 为 105.293448/102.468848 秒，
  summary SHA-256 为 `7b400d16ee1f732a381db0166f3bdec2e960dbeda79b4e87d8ed6e9ca1bfec9b` /
  `99640d9830ff8c2fc229887134fa2636ae8e66d73b63a2b73cbc19980c6e859c`，console 为
  `c48a12ada4d0ee21dc5c52959fd9e4382bef51147012070f672014e61849ebd0` /
  `4b7ab469ac0b316f2f6a808e426aef99b684d584f307739a5a9a3eab863a14f0`；RED 不计 PASS。
- `26485a56407758b701742094e8e513977ddf3ede` 把 timed-command timeout 单次解析移到
  `sys_futex`：未知 command/flag 不读取 timeout；WAIT 使用原有 relative restart deadline
  机制解析，其余四类 timed command 使用 absolute timespec validation；解析后才执行未实现
  operation/clock/source validation。`wait_addr` 只消费 `Option<Duration>`，因此 source word
  不再遮蔽 timeout errno，也不发生第二次用户内存读取。relative state 在 timeout 错误、
  unsupported operation、alignment、source fault/mismatch 与正常完成路径均精确清理。三类
  未实现 PI command 在 timeout 合法后仍返回 `ENOSYS`，没有伪实现。
- 四项 scoped mutation 约束完整 timed-command 集合、dispatcher 顺序和 wait helper 不得延迟
  读取 timeout。首次 focused unit 因旧的 operation-order mutation fixture 未包含新增
  `finish_relative_futex_wait` 行而 38/39 FAIL；修正后 39/39，通过扩展第五类 timed command
  guard 后最终 40/40 PASS。semantic unit 75/75、no-fake/boundary guard 0 finding、RV/LA
  build 均 PASS（既有 175 warnings）。expanded dirty GREEN console SHA-256 为
  `3e150526cea6270ec8cbb4afad9c097f3db81075fd1afc68bf11eb00b73fb8cd` /
  `d24463f5e3083fc6be40389ef85f57c6f490d1d77377759f985774826a824e60`，均完整出现新 marker、
  USER/HARNESS PASS 和 SHUTDOWN；dirty 结果只作诊断。
- `73b5eafc3c5b79eaaf9c14e4edba67b1e6e8d90e` 把 syscall mutation pin 从 36 收紧到
  40，asset pin 保持 36；组合单测 76/76、完整 suite list、runner 135/135 均 PASS。clean
  RV/LA canonical runtime 各 1/1 PASS、零非 PASS，outer/inner provenance clean/stable，
  required 3/3 PASS。RV summary/inner/console SHA-256 为
  `e84890b12bf3c91e748a24e9a14df1e8ae6804a4da4498cc4156b2283546e84e` /
  `e5119f89948b327ed16c7f5d3c24e4e86d8689193a80ed661687df7e2c20ebb8` /
  `2c18e17a571b62d421549df5d1c3613be46ef0815d0514b470b73e6b57c2ad1e`；LA 为
  `49cfa02d73a25c60feb1adfe4216a80f2d26a7ce5ce461ec938ab1269a846f52` /
  `ad6645380828b0dc0d78a8f72198e7f42d607afbcdb8089d7030c5e0d2b7401d` /
  `15f29a6c1d04d41b7a34dd60d76c04eb0c0ca879286524c1f4f6235328535963`；duration
  104.239222/101.444135 秒。canonical quick 47/47、mutation 40/40、asset 36/36 PASS，
  duration 341.400846 秒，summary SHA-256
  `23add8bcf8903f0141f7c0c0a4f9fcd074946df7a6a281e3f5012096dbeb6a1c`。
- 本 checkpoint 证明 timed futex timeout copy-in 与已覆盖 dispatcher/early-return 顺序，未
  运行正式 final rootfs 或推导 CAgent/BuildStorm 分数。raw 64-bit syscall register 到 Linux
  `u32 val/val2/val3` 的截断边界仍需下一 checkpoint 独立审计。
...
| OpenAI Codex / GPT-5 系列（精确子版本未知） | Checkpoint 39 host/Linux CMP requeue 地址验证与 errno 顺序审计、合同、双架构 RED/GREEN、修复与记录 | `semantic_smoke.rs`、semantic manifest/fixture、`futex.rs`、boundary guard/unit、suite manifest/runner pin、计划与本日志 | 采用 null/unmapped/misaligned/valid 四分支真实 syscall；保留错误 make 目标和首次并行 LA guest timeout；仅修复确定性验证顺序，不宣称 compare/requeue 原子性 | host 原始 syscall、Linux exact source、guard、29/29 mutation、75/75 semantic、双架构 build、dirty RED/GREEN、36/36+135/135 注册回归、clean canonical RV/顺序 LA runtime/quick 与 provenance；最终仍需人工复核 | 待人工负责人确认 |
| OpenAI Codex / GPT-5 系列（精确子版本未知） | Checkpoint 40 Linux CMP requeue queue-lock 原子区审计、通用 checked WaitQueue 设计、实现与记录 | `wait_queue.rs`、axtask tests、`futex.rs`、boundary guard/unit、suite manifest/runner pin、计划与本日志 | 采用稳定锁序下 fallible check 与失败零 mutation；保留 parser/fixture 首轮 2 FAIL 和 replacement 首轮 1 FAIL；拒绝概率竞态假 RED、固定成功或特化 | Linux exact source、32/32 mutation、75/75 semantic、axtask 5+2 tests、双架构 build、dirty GREEN、36/36+135/135 注册回归、clean canonical RV/LA runtime/quick 与 provenance；最终仍需人工复核 | 待人工负责人确认 |
| OpenAI Codex / GPT-5 系列（精确子版本未知） | Checkpoint 41 host/Linux futex source errno 审计、双架构 RED/GREEN、最小实现与记录 | `semantic_smoke.rs`、semantic manifest/fixture、`futex.rs`、boundary guard/unit、suite manifest/runner pin、计划与本日志 | 采用 NULL WAIT/WAKE、misaligned/valid 控制；保留 unsupervised raw 与错误 asset 路径两次 pre-test exit 2；拒绝固定 errno、程序/路径特化 | host 原始 syscall、Linux exact source、33/33 mutation、75/75 semantic、双架构 build、clean RED、dirty GREEN、36/36+135/135 注册回归、clean canonical RV/LA runtime/quick 与 provenance；最终仍需人工复核 | 待人工负责人确认 |
| OpenAI Codex / GPT-5 系列（精确子版本未知） | Checkpoint 42 host/Linux futex operation/flag errno 与验证顺序审计、双架构 RED/GREEN、实现与记录 | `semantic_smoke.rs`、semantic manifest/fixture、`futex.rs`、boundary guard/unit、suite manifest/runner pin、计划与本日志 | 采用三类 realtime command、未知 flag/command、非法 source 优先级与受支持 WAIT_BITSET 控制；保留首次 manifest 计数错绑的 infrastructure error；拒绝固定 errno、程序/路径/架构特化 | host 原始 syscall、Linux exact source、36/36 mutation、75/75 semantic、双架构 build、clean RED、dirty GREEN、36/36+135/135 注册回归、clean canonical RV/LA runtime/quick 与 provenance；最终仍需人工复核 | 待人工负责人确认 |
| OpenAI Codex / GPT-5 系列（精确子版本未知） | Checkpoint 43 host/Linux timed futex timeout copy-in/errno 顺序审计、双架构 RED/GREEN、实现与记录 | `semantic_smoke.rs`、semantic manifest/fixture、`futex.rs`、boundary guard/unit、suite manifest/runner pin、计划与本日志 | 采用坏 pointer/非法 nsec、mismatch/misaligned/realtime/bitset/PI/未知 command 控制；保留首次 focused fixture 38/39 FAIL；拒绝重复 copy-in、未实现 PI 假成功与程序/路径/架构特化 | host 原始 syscall、Linux exact source、40/40 mutation、75/75 semantic、双架构 build、clean RED、dirty GREEN、76/76+135/135 注册回归、clean canonical RV/LA runtime/quick 与 provenance；最终仍需人工复核 | 待人工负责人确认 |
...
| Linux kernel + kernel docs | `b95f03f04d475aa6719d15a636ddf32222d55657`，`kernel/futex/requeue.c`、`core.c`、`syscalls.c`；v6.12 locking 文档 | 只读核验普通 requeue 返回 requeued 或 woken 的任务总数、同 bucket 不移动链表但仍更新 key/计数、CMP requeue 在 bucket locks 下比较、`get_futex_key()` 的 alignment `EINVAL`/access `EFAULT` 顺序、未知 operation/realtime flag 的 `ENOSYS` 分派，以及 `futex_cmd_has_timeout()`/`futex_init_timeout()` 的 pre-dispatch timeout copy-in；并与 host 原始 syscall 交叉验证 | GPL-2.0-only（kernel）；文档原许可；未复制实现代码 | 独立 Rust 计数/同队列保留/地址、operation 和 timeout 验证顺序、checked queue-lock 临界区、source errno 与 operation `ENOSYS` 修正、行为测试和 scoped mutation guard；未复制 Linux 算法，多 waiter/信号/超时压力仍作为未解决项 | `user/shell/src/uspace/futex.rs`、`kernel/task/axtask/src/wait_queue.rs`、本日志 |
...
| `ebc0de86` | RV/LA CMP requeue validation dirty supervised RED | QEMU 0 / 0，guest nonzero | FAIL / FAIL；均先过普通/同地址 requeue，再明确 `USER_FAIL futex_cmp_requeue_validation`，无 timeout/panic/trap | RED log SHA-256 `e370932e...4536` / `b347e120...c54`；仅定向诊断，不作为 canonical PASS |
| `e71c46cf` | CMP requeue focused guards/unit + RV/LA smoke builds | 0 | PASS，boundary/no-fake guard 0 finding、29/29 mutation、75/75 semantic、双架构 build | null/unmapped EFAULT、misaligned EINVAL、valid mismatch EAGAIN；错误 make 目标 exit 2 单列保留；dirty GREEN `18997e1e...b3a9` / `6a352080...7423` |
| `07ff1981` | mutation identity/runner/list/asset registration | 0 | PASS，29/29 + 135/135、36/36 asset、完整 suite list | 两处 versioned count 从 27 收紧到 29 |
| `07ff1981` | 首次并行 clean RV/LA canonical evidence-runtime | 0 / 1 | RV PASS 1/1；LA FAIL 1/1，`guest_timeout`，未覆盖 | 177.742963 / 175.754790 s；summary SHA-256 `223ad0ff...f744` / `47c64695...ea8e`；`20260721T145920Z...` |
| `07ff1981` | 顺序 LA canonical evidence-runtime 诊断重跑 | 0 | PASS，1/1，零非 PASS，outer/inner provenance clean/stable；guest 明确新 marker PASS | 102.656358 s；summary `1a7597af...6238`、console `8ca90cd3...a8fa`；`20260721T150247Z...`；不覆盖首次 FAIL |
| `07ff1981` | canonical quick | 0 | PASS，47/47，零非 PASS，provenance clean/stable | 340.548009 s；summary SHA-256 `ec412d1a...b4c7`；`20260721T150445Z-quick-none-3732224/` |
| `c59e2577` | CMP atomicity focused guards/unit + axtask + RV/LA smoke builds | 0 | PASS，boundary/no-fake guard 0 finding、32/32 mutation、75/75 semantic、axtask 5/5 unit + 2/2 doc、双架构 build | 首轮 parser/fixture 2 FAIL、修正后 replacement 1 FAIL 均保留；dirty sequential GREEN `663cd882...b2b` / `c5000a97...eec` |
| `7aa94639` | mutation identity/runner/list/asset registration | 0 | PASS，32/32 + 135/135、36/36 asset、完整 suite list | 三项新增 scoped mutation，runner/manifest pin 从 29 收紧到 32 |
| `7aa94639` | RV/LA canonical evidence-runtime | 0 / 0 | PASS，1/1 + 1/1，零非 PASS，outer/inner provenance clean/stable、required 3/3 PASS | 104.896086 / 101.688061 s；summary `fe296874...9b7` / `accfd353...488`；console `b835040a...3a4` / `b8c35555...9af` |
| `7aa94639` | canonical quick | 0 | PASS，47/47，零非 PASS，provenance clean/stable，精确执行 32 项 syscall mutation | 340.000249 s；summary SHA-256 `f916614d...187`；`20260721T153341Z-quick-none-3748971/` |
| `28a4b3a7` | RV/LA futex source validation clean canonical RED | 1 / 1 | FAIL / FAIL；此前 futex marker 均 PASS，随后明确 `USER_FAIL futex_source_validation`、guest nonzero 与正常关机，无 timeout/panic/trap | 104.810349 / 102.023393 s；summary `423e69b5...e7d` / `ba626f36...c09`；console `400d09bd...487` / `4a0c7987...71d` |
| `d1652414` | source errno focused guards/unit + RV/LA smoke builds | 0 | PASS，boundary/no-fake guard 0 finding、33/33 mutation、75/75 semantic、双架构 build | dirty sequential GREEN `8617acd5...7c0` / `3ca8534b...1c8`；无新增 production unsafe/依赖/ABI/特化 |
| `ed4cba1d` | mutation identity/runner/list/asset registration | 0 | PASS，33/33 + 135/135、36/36 asset、完整 suite list | runner/manifest pin 从 32 收紧到 33；两次 pre-test exit 2 单列保留 |
| `ed4cba1d` | RV/LA canonical evidence-runtime | 0 / 0 | PASS，1/1 + 1/1，零非 PASS，outer/inner provenance clean/stable、required 3/3 PASS | 105.908556 / 102.082055 s；summary `028941d5...0ff` / `8c9fc209...220`；console `b7223dfb...387` / `e0195ce7...0e6` |
| `ed4cba1d` | canonical quick | 0 | PASS，47/47，零非 PASS，provenance clean/stable，精确执行 33 项 syscall mutation | 344.357334 s；summary SHA-256 `69004df3...8ed`；`20260721T160450Z-quick-none-3768265/` |
| `fb4028ed` | RV/LA futex operation validation clean canonical RED | 1 / 1 | FAIL / FAIL；此前 marker 均 PASS，随后明确 `USER_FAIL futex_operation_validation`、guest nonzero 与正常关机，无 timeout/panic/trap | 105.329857 / 101.557944 s；summary `5b1250ee...f27` / `b21f3fe9...f5e`；console `f88d8245...b90` / `e2b3359c...495` |
| `f45334cc` | operation validation focused guards/unit + RV/LA smoke builds | 0 | PASS，boundary/no-fake guard 0 finding、36/36 mutation、75/75 semantic、双架构 build | dirty sequential GREEN `63af8607...a39` / `99edb62a...1bd`；无新增 production unsafe/依赖/ABI/特化 |
| `677bfbcf` | mutation identity/runner/list/asset registration | 首次 2，修正后 0 | 首次 suite list 因 competition count 错绑而 INFRA_ERROR、0 case；修正后 36/36 + 135/135、36/36 asset、完整 list PASS | syscall pin 从 33 收紧到 36；首次错误未计 PASS |
| `677bfbcf` | RV/LA canonical evidence-runtime | 0 / 0 | PASS，1/1 + 1/1，零非 PASS，outer/inner provenance clean/stable、required 3/3 PASS | 104.102379 / 101.163025 s；summary `73cb1d64...cb6` / `dd41b683...ab6`；console `349df719...cee` / `ffd9eb55...630` |
| `677bfbcf` | canonical quick | 0 | PASS，47/47，零非 PASS，provenance clean/stable，精确执行 36 项 syscall mutation | 342.099651 s；summary SHA-256 `c9dd158e...f59`；`20260721T163709Z-quick-none-3787484/` |
| `ed46bf17` | RV/LA futex timeout validation clean canonical RED | 1 / 1 | FAIL / FAIL；source/operation marker PASS 后明确 `USER_FAIL futex_timeout_validation`、guest nonzero 与正常关机，无 timeout/panic/trap | 105.293448 / 102.468848 s；summary `7b400d16...c9b` / `99640d98...59c`；console `c48a12ad...bd0` / `4b7ab469...14f` |
| `26485a56` | timeout validation focused guards/unit + RV/LA smoke builds | 0 | PASS，boundary/no-fake guard 0 finding、40/40 mutation、75/75 semantic、双架构 build | 首次 fixture 38/39 FAIL 如实保留；expanded dirty GREEN `3e150526...8cd` / `d24463f5...e60`；无新增 production unsafe/依赖/ABI/特化 |
| `73b5eafc` | mutation identity/runner/list/asset registration | 0 | PASS，40/40 + 135/135、36/36 asset、完整 suite list | syscall pin 从 36 收紧到 40，asset pin 保持 36 |
| `73b5eafc` | RV/LA canonical evidence-runtime | 0 / 0 | PASS，1/1 + 1/1，零非 PASS，outer/inner provenance clean/stable、required 3/3 PASS | 104.239222 / 101.444135 s；summary `e84890b1...84e` / `49cfa02d...f52`；console `2c18e17a...d1e` / `15f29a6c...963` |
| `73b5eafc` | canonical quick | 0 | PASS，47/47，零非 PASS，provenance clean/stable，精确执行 40 项 syscall mutation | 341.400846 s；summary SHA-256 `23add8bc...a1c`；`20260721T171937Z-quick-none-3811957/` |
...
同地址 requeue 已由 `e4b693c8` -> `673cb888`/`0c6069af` 形成双架构 RED/GREEN，CMP requeue
地址验证/errno 顺序已由 `ebc0de86` -> `e71c46cf`/`07ff1981` 闭合，queue-lock 下的原子比较又由
`c59e2577`/`7aa94639` 以通用 checked requeue 与 mutation guard 闭合；仓内继续审计多 waiter、
地址 unmap/remap、signal/timeout 与调度压力，并以通用可复现行为合同限定每项结论，不把当前
checkpoint 外推为完整并发压力或正式 toolchain 通过。futex source NULL errno 又由
`28a4b3a7` -> `d1652414`/`ed4cba1d` 形成确定性 host + 双架构 RED/GREEN；继续审计其余真实
ABI/errno 和动态 toolchain 阻塞，不以未运行的正式镜像推导分数。未知 flag/command、非法
realtime 组合与 source address 前的 `ENOSYS` 优先级又由 `fb4028ed` ->
`f45334cc`/`677bfbcf` 闭合；timed command 的 timeout copy-in 相对 operation/source validation
的 errno 优先级再由 `ed46bf17` -> `26485a56`/`73b5eafc` 闭合。下一项独立审计 raw 64-bit
syscall 参数到 Linux `u32 val/val2/val3` 的截断边界，不能从当前 typed compare 外推为已正确。
