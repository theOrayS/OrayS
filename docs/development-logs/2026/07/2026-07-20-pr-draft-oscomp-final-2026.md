...
| Linux kernel + kernel docs | `b95f03f04d475aa6719d15a636ddf32222d55657`，`kernel/futex/requeue.c`；v6.12 locking 文档 | 只读核验普通 requeue 返回 requeued 或 woken 的任务总数、同 bucket 不移动链表但仍更新 key/计数，以及 CMP requeue 先取得两个 key、再于 bucket locks 下比较；并与 host 原始 syscall 交叉验证 | GPL-2.0-only（kernel）；文档原许可；未复制实现代码 | 独立 Rust 计数/同队列保留/地址验证顺序与 checked queue-lock 临界区、行为测试和 scoped mutation guard；未复制 Linux 算法，多 waiter/信号/超时压力仍作为未解决项 | `user/shell/src/uspace/futex.rs`、`kernel/task/axtask/src/wait_queue.rs`、本日志 |
...
checkpoint 外推为完整并发压力或正式 toolchain 通过。
