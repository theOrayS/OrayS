...
| Linux kernel + kernel docs | `b95f03f04d475aa6719d15a636ddf32222d55657`，`kernel/futex/requeue.c`、`core.c`、`syscalls.c`；v6.12 locking 文档 | 只读核验普通 requeue 返回 requeued 或 woken 的任务总数、同 bucket 不移动链表但仍更新 key/计数、CMP requeue 在 bucket locks 下比较、`get_futex_key()` 的 alignment `EINVAL`/access `EFAULT` 顺序，以及未知 operation/realtime flag 的 `ENOSYS` 分派；并与 host 原始 syscall 交叉验证 | GPL-2.0-only（kernel）；文档原许可；未复制实现代码 | 独立 Rust 计数/同队列保留/地址和 operation 验证顺序、checked queue-lock 临界区、source errno 与 operation `ENOSYS` 修正、行为测试和 scoped mutation guard；未复制 Linux 算法，多 waiter/信号/超时压力仍作为未解决项 | `user/shell/src/uspace/futex.rs`、`kernel/task/axtask/src/wait_queue.rs`、本日志 |
...
`f45334cc`/`677bfbcf` 闭合；下一项独立核验 timeout copy-in 相对 operation/source validation
的 Linux errno 优先级，不能从 Checkpoint 42 外推为已经正确。
