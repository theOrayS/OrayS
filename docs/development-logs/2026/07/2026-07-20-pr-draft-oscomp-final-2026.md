...
| Linux kernel + kernel docs | `b95f03f04d475aa6719d15a636ddf32222d55657`，`kernel/futex/requeue.c`、`core.c`；v6.12 locking 文档 | 只读核验普通 requeue 返回 requeued 或 woken 的任务总数、同 bucket 不移动链表但仍更新 key/计数、CMP requeue 在 bucket locks 下比较，以及 `get_futex_key()` 的 alignment `EINVAL`/access `EFAULT` 顺序；并与 host 原始 syscall 交叉验证 | GPL-2.0-only（kernel）；文档原许可；未复制实现代码 | 独立 Rust 计数/同队列保留/地址验证顺序、checked queue-lock 临界区与 source errno 修正、行为测试和 scoped mutation guard；未复制 Linux 算法，多 waiter/信号/超时压力仍作为未解决项 | `user/shell/src/uspace/futex.rs`、`kernel/task/axtask/src/wait_queue.rs`、本日志 |
...
ABI/errno 和动态 toolchain 阻塞，不以未运行的正式镜像推导分数。
