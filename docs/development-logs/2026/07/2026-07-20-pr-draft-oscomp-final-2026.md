...
| Linux kernel + kernel docs | `b95f03f04d475aa6719d15a636ddf32222d55657`，`kernel/futex/requeue.c`；v6.12 locking 文档 | 只读核验普通 requeue 返回 requeued 或 woken 的任务总数，并与 host 原始 syscall 交叉验证 | GPL-2.0-only（kernel）；文档原许可；未复制实现代码 | 独立 Rust 计数修正、行为测试与 scoped mutation guard；未复制 Linux 算法 | `user/shell/src/uspace/futex.rs`、本日志 |
...
形成双架构 RED/GREEN；仓内继续审计 CMP requeue 原子比较、same-address、signal/timeout 与
比较-排队竞争，并以通用可复现行为合同限定每项结论，不把当前 checkpoint 外推为完整并发
压力或正式 toolchain 通过。
