...
| Linux man-pages | Ubuntu `manpages-dev 5.10-1ubuntu1`，`sigaltstack(2)` | 只读核验成功 exec 清除、fork 复制及 `CLONE_VM`/`CLONE_VFORK` 例外 | man-pages 页面内许可；未复制实现代码 | 独立 Rust 状态转换和原始 syscall 测试，仅重述公开 Linux 语义 | `user/shell/src/uspace/task_context.rs`、`process_lifecycle.rs`、本日志 |
