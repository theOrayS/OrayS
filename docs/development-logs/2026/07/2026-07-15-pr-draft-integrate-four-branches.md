...
窄修复及 fresh smoke/full 已完成，但在 follow-up review 前不得写为清零或通过。
...
- 首轮独立只读 AI reviewer 已完成；unsafe-contract 修复已提交，fresh 双架构
  smoke 通过且 full 已完成，但其 1 个 Major 在 follow-up review 前尚未清零；人类
  负责人也尚未确认可独立解释和调试。
...
首轮 reviewer 的 unsafe 契约与 active-plan 窄修复、fresh RV64/LA64 smoke 和
full/all 已完成；下一步由同一独立 reviewer follow-up 确认 Major/Minor 处置。其后
仍须修正外部 BusyBox plan并通过受控 provenance 重新生成两架构 image，
...
1 Major、1 Minor；unsafe 契约/计划修复已提交且 fresh 双架构 smoke 通过，fresh full
仍为 57 PASS + 2 INFRA_ERROR，follow-up review 尚未完成。因此 `main`
