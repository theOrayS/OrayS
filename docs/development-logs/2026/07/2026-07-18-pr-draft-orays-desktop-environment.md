...
## 2026-07-20 post-merge 独立复审 blocker 整改

最新输入为 `/tmp/codex-post-merge-review-fixes.txt`。本轮保留 HEAD `d9891d02` 及用户明确
授权的三个本地提交，不做 reset/revert/rebase，不操作 `main`/本地 `stabilize`，不修改
`tee_device_mode`，不 commit/push、不触发 GitHub Actions，也不设置或恢复 Goal。

测试先行记录：scope 的特殊 pathname/缺 base 用例先因 Unicode decode、pathname strip 和 HEAD
fallback 失败；runtime identity/helper/policy 缺失用例先失败；metadata→summary→package 链的
identity tamper 先被旧 schema/缺字段暴露；runner 行为用例先证明缺 identity 参数、SIGINT/TERM
残留子进程及 output no-evidence contract 缺失；同冒号前缀的伪失败细节最初被 validator 错误
接受为 `VALID_FAIL`；workflow 去自托管测试当前仍保持 RED。修复后前六类均已 GREEN。

实现范围：

- scope 全程使用 NUL-delimited bytes，pathname 不做 strip；baseline 只接受显式 base 或可验证的
  `origin/develop/post-integration-next`；
- 新增 tracked `runtime-policy.json`、`runtime-identity.py` 与 `runtime-watchdog.py`；两架构 QEMU
  分别固定 approved digest，执行前通过 canonical path 打开、核验同一对象并从
  `/proc/self/fd` exec；
- metadata/summary/package schema 为 4/3/4，完整记录 QEMU required/observed digest、artifact
  digest/source/path、build invocation、完整 QEMU argv；pre-start FAIL 只允许不声称 observation 的
  partial identity；
- runner 使用直系 session leader 和单次 wait，watchdog 对 timeout 做进程组 TERM/KILL；finalizer
  对 build fail、早退、timeout 与 signal 生成可迁移 `VALID_FAIL`，只有 output 或 Python 根本不可用
  时使用明确 `DESKTOP_RUNTIME_EVIDENCE=UNAVAILABLE` stderr/exit contract；
- validator 精确比较完整有序失败列表，不再比较冒号类别；异常中的可迁移 run 根规范化为
  `<run-dir>`，同前缀伪 details 即使重算外层哈希仍被拒绝。

本轮实际验证：

| 命令 | 退出码/结果 |
| --- | --- |
| `python3 -B -m unittest discover -s test/desktop -p test_*.py` | 1；87 项，86 PASS / 1 workflow RED |
| `scripts/desktop/build.sh host-test` | 0；73/73 PASS |
| host clippy `--all-targets --features host-tools -- -D warnings` | 0 |
| desktop rustfmt、asset/license、golden、desktop shell syntax | 0；golden 5/5 |
| `python3 -B scripts/desktop/check-scope.py` | 0；119/3/3/74 |
| `git diff --check` | 0 |

唯一当前 Python 失败要求 `.github/workflows/desktop.yml` 不含 `FIXED_QEMU_VERSION`、
`desktop-runtime` 或 `self-hosted`。按复审输入提交的最小删除补丁被权限审查拒绝，文件未改变；
不得绕过。历史“trusted branch/self-hosted runtime”仅为旧 checkpoint 记录，不是本轮完成声明，
当前远端 CI 继续标记 deferred/unverified。

最终 clean snapshot、RV/LA 五场景 10/10 及 relocated 10/10、canonical quick 尚未重跑。原因是
用户同时要求 live dirty 且禁止 commit；没有 clean commit 就不能诚实产生
`source_dirty=false`/`provenance_stable=true` 的 PASS package。本轮不把 dirty run、official/tee
未运行或历史证据表述为当前 PASS。AI 使用：OpenAI Codex 用于 blocker 映射、RED/GREEN 测试、
identity/evidence/runner 实现和本记录；仍需独立只读复审最终 diff。
