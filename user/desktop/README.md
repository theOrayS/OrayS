...
- Terminal：输入、历史、有界输出、滚动，以及作用于真实文件系统的
  `pwd/ls/cd/cat/mkdir/write`；
...
input/capture transcripts、输入序列、PPM、SHA-256 与 fail-closed summary。缺 QEMU、截图、
输入 transcript、来宾 marker 或正常退出中的任一项都会失败。
