...
pub fn with_ordered_arc_mutex_pair<T, R>(
    ...
    let first_rank = Arc::as_ptr(first) as usize;
    let second_rank = Arc::as_ptr(second) as usize;
    if first_rank < second_rank {
        let mut first_guard = first.lock();
        let mut second_guard = second.lock();
        Some(operation(&mut first_guard, &mut second_guard))
    } else {
        let mut second_guard = second.lock();
        let mut first_guard = first.lock();
        Some(operation(&mut first_guard, &mut second_guard))
...
struct EventSourceInner {
    observers: Mutex<BTreeMap<EventToken, Weak<dyn EventObserver>>>,
...
mod tests {
    ...
    struct DropTargetObserver(Mutex<Option<Arc<()>>>);
    ...
    fn legacy_target_lease_survives_fork_alias_and_drops_on_final_close() {
        ...
        let observer = Arc::new(DropTargetObserver(Mutex::new(Some(target))));
        let erased: Arc<dyn EventObserver> = observer.clone();
        let (_, _subscription) = identity.close_events().subscribe(&erased).unwrap();
    ...
    fn one_shot_claim_is_single_winner_across_concurrent_waiters() {
        let delivery = Arc::new(Mutex::new(EventDeliveryState::default()));
    ...
    fn reciprocal_pair_operations_share_one_lock_order() {
        let first = Arc::new(Mutex::new(1_u32));
        let second = Arc::new(Mutex::new(10_u32));

        with_ordered_arc_mutex_pair(&first, &second, |source, destination| {
            *destination += *source;
        })
        .unwrap();
        with_ordered_arc_mutex_pair(&second, &first, |source, destination| {
            *destination += *source;
        })
        .unwrap();
        ...
        assert!(with_ordered_arc_mutex_pair(&first, &first, |_, _| ()).is_none());
    }

    #[test]
    fn object_state_is_shared_only_by_description_aliases() {
        let description = OpenFile::new(Mutex::new(0_u32), 0).unwrap();
        let alias = Arc::clone(&description);
        let independent = OpenFile::new(Mutex::new(0_u32), 0).unwrap();
