#!/usr/bin/env python3
"""Static guard for compatibility behavior backed by authoritative kernel state."""
...
def scan_scheduler_backend_effect(root: Path) -> list[str]:
    ...
    live_task_helper = rust_function_block(text, "apply_process_scheduler_state_to_live_tasks")
    current_is_applied = (
        "apply_process_scheduler_state_to_live_tasks(process, state)" in setter
        and "apply_task_scheduler_state(current.as_task_ref(), process, state)" in live_task_helper
    )
    ...
    for token in (
        ...
        if token not in rr:
            findings.append(f"vendor/axsched/src/round_robin.rs: RR backend must age skipped lower-priority tasks to avoid starvation; missing {token}")
    for token in (
...
def main() -> int:
    ...
    if findings:
        print("kernel state-backed semantics check: FAIL")
        for finding in findings:
            print(f"- {finding}")
        return 1
    print("kernel state-backed semantics check: PASS (0 findings)")
    return 0
