#!/usr/bin/env python3
"""Static regression guard for fake-success regressions.

The check is intentionally narrow: it looks only at the fake-success surfaces
...
def scan_repo(root: Path = REPO_ROOT) -> list[Finding]:
    ...
    axlibc_paths = sorted(root.glob(AXLIBC_C_GLOB))
    if not axlibc_paths:
        findings.append(
            Finding(
                Path(AXLIBC_C_GLOB),
                1,
                "required-source-set-empty",
                "no axlibc C sources matched the required scan input",
            )
        )
    for path in axlibc_paths:
        findings.extend(scan_c_unimplemented_return_success(path, path.read_text(), root))

    api_paths = sorted(root.glob(API_IMP_GLOB))
    if not api_paths:
        findings.append(
            Finding(
                Path(API_IMP_GLOB),
                1,
                "required-source-set-empty",
                "no POSIX API implementation sources matched the required scan input",
            )
        )
    for path in api_paths:
        findings.extend(scan_api_fake_success(path, path.read_text(), root))

    fd_table = root / FD_TABLE_PATH
    if not fd_table.is_file():
        findings.append(
            Finding(
                FD_TABLE_PATH,
                1,
                "required-source-file-missing",
                "descriptor-table source required by the guard is missing",
            )
        )
    else:
        findings.extend(scan_fd_table_fake_success(fd_table, fd_table.read_text(), root))
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.json:
        ...
        print(f"no fake success static check: FAIL ({len(findings)} findings)")
        for finding in findings:
            print(finding.display())
    else:
        print("no fake success static check: PASS (0 findings)")
