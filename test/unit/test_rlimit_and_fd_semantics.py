#!/usr/bin/env python3
"""Unit tests for the rlimit-and-fd rlimit/sysconf/fcntl/FD static guard."""
...
import check_rlimit_and_fd_semantics as guard


class RlimitAndFdSemanticsGuardTest(unittest.TestCase):
...
''',
        ...
        self.assertEqual(findings[0].kind, "rlimit-and-fd-sysconf-fallback-ok-zero")
...
''',
        ...
        self.assertTrue(any(f.kind == "rlimit-and-fd-rlimit-unsupported-ok-zero" for f in findings))
...
''',
        ...
        self.assertTrue(any(f.kind == "rlimit-and-fd-api-filelike-default-status-ok-zero" for f in findings))
...
''',
        ...
        self.assertIn("rlimit-and-fd-api-fcntl-missing-fd-command", kinds)
...
''',
        ...
        self.assertTrue(any(f.kind == "rlimit-and-fd-api-dupfd-cloexec-drops-flag" for f in findings))
...
''',
        ...
        self.assertTrue(any(f.kind == "rlimit-and-fd-prlimit-invalid-resource-success" for f in findings))
...
''',
        ...
        kinds = {f.kind for f in findings}
        self.assertIn("rlimit-and-fd-fd-table-limit-default-nofile", kinds)
        self.assertIn("rlimit-and-fd-fd-table-imports-default-nofile", kinds)
...
''',
        ...
        self.assertTrue(any(f.kind == "rlimit-and-fd-shell-fcntl-fallback-ok-zero" for f in findings))
