#!/usr/bin/env python3
"""Regression tests for the Linux boundary static guard."""
...
GUARD = ROOT / "test/checks/check_linux_boundary.py"
TARGETS = (
...
SPEC = importlib.util.spec_from_file_location("check_linux_boundary", GUARD)
assert SPEC is not None and SPEC.loader is not None
GUARD_MODULE = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(GUARD_MODULE)


class LinuxBoundaryGuardTest(unittest.TestCase):
    def make_tree(self) -> Path:
        tmp = Path(tempfile.mkdtemp(prefix="linux-boundary-"))
