...
impl<T> BaseScheduler for CFScheduler<T> {
    ...
    fn set_priority(&mut self, task: &Self::SchedItem, prio: isize) -> bool {
        if (-20..=19).contains(&prio) {
            let queued_key = self.ready_queue.iter().find_map(|(key, queued)| {
                if Arc::ptr_eq(queued, task) {
                    Some(*key)
                } else {
                    None
                }
            });
            let queued_task = queued_key.and_then(|key| self.ready_queue.remove(&key));
            task.set_priority(prio);
            if let Some(queued_task) = queued_task {
                let taskid = queued_task.get_id();
                self.ready_queue
                    .insert((queued_task.get_vruntime(), taskid), queued_task);
                self.refresh_min_vruntime();
            }
            true
        } else {
