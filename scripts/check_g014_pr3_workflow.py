...
def scan_test_workflow(path: Path, text: str) -> list[str]:
    ...
    findings.extend(
        require_tokens(
            ...
            (
                QEMU_SOURCE_SHA256,
                "bash scripts/setup_qemu_pr3.sh",
                "--arch ${{ matrix.arch }}",
                "--arch host",
                "build/pr3-shards/host/semantic-evidence-v1.json",
                "build/pr3-shards/rv64/semantic-evidence-v1.json",
                "build/pr3-shards/la64/semantic-evidence-v1.json",
                "scripts/render_semantic_evidence.py",
    ...
    for token in (
        "libfdt-dev",
        "python3 scripts/semantic_evidence.py supervise",
        "--timeout 4800",
...
def scan_repository_contracts(root: Path, test_text: str) -> list[str]:
    ...
    try:
        ...
        setup = (root / "scripts" / "setup_qemu_pr3.sh").read_text(encoding="utf-8")
        evidence_docs = (root / "docs" / "pr3-semantic-evidence.md").read_text(
            encoding="utf-8"
        )
        manifest = json.loads(
            (root / "scripts" / "semantic_evidence_manifest.json").read_text(
                encoding="utf-8"
            )
        )
    ...
    for capability_id, executable in (
        ...
        if capability != expected:
            findings.append(
                "scripts/semantic_evidence_manifest.json: "
    ...
    for token in ("--disable-download", "--disable-slirp"):
        if setup.count(token) < 2:
            findings.append(
                f"scripts/setup_qemu_pr3.sh: missing offline configure token {token}"
            )
    ...
    if active_archive_extractors != [safe_archive_extractor]:
        findings.append(
            "scripts/setup_qemu_pr3.sh: exactly one active archive extraction must "
            "use --no-same-owner"
        )
    ...
    for token in ("set +e", "pr3-evidence-host pr3-evidence-rv pr3-evidence-la", "pr3-render-required"):
        if token not in required_local:
            findings.append(f"Makefile: pr3-evidence-required missing always-collect token: {token}")
    return findings
...
def main() -> int:
    ...
    if findings:
        print("G014 PR3 workflow policy: FAIL")
        for finding in findings:
            print(f"- {finding}")
        return 1
    print("G014 PR3 workflow policy: PASS (0 findings)")
    return 0
