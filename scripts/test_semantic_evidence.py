...
FIXTURES = SCRIPT_DIR / "fixtures" / "semantic-evidence"
...
def result_case(
    ...
    *,
    ...
    return {
        ...
        "provenance": ["scripts/test_semantic_evidence.py"],
    }
...
class ManifestTests(unittest.TestCase):
    ...
    def test_repository_manifest_expands_complete_guard_inventory(self) -> None:
        ...
        self.assertEqual(len(manifest["expanded_cases"]), 40)
        ids = [case["id"] for case in manifest["expanded_cases"]]
        self.assertIn("guard.g002-fake-success.check", ids)
        self.assertIn("guard.selfcheck-compliance-fixes.mutations", ids)
                ...
                "inventories": [
                    {
                        ...
                        "check_glob": "scripts/check_g*.py",
                        "test_glob": "scripts/test_g*.py",
                        "runner_id": "static",
                        "category": "compliance",
                        "policy": "required",
                        "timeout_seconds": 10,
                        "requires": ["python3"],
                        "expected_ids": ["g002", "g003"],
                    }
                ],
                "cases": [],
            }
            with self.assertRaisesRegex(evidence.ManifestError, "missing expected guard ids: g002"):
...
class ClassifierTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.g011 = next(
            ...
            if case["id"] == "guard.g011-empty-shells.check"
        )
    ...
    def test_complete_runtime_protocol_passes(self) -> None:
        result = self.classify("smoke-rv64-pass.log")
    ...
    def test_guard_prefix_pass_protocol_is_accepted_once(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            log.write_text(
                "PASS: G011 empty-shell guard found no success-without-state shells\n",
                encoding="utf-8",
            )
    ...
    def test_guard_duplicate_prefix_pass_protocol_is_ambiguous(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            log = Path(temporary) / "guard.log"
            line = "PASS: G011 empty-shell guard found no success-without-state shells\n"
    ...
    def test_guard_protocol_rejects_non_ascii_text(self) -> None:
        pass_line = b"PASS: G011 empty-shell guard found no success-without-state shells\n"
    ...
    def test_display_ignorable_bytes_cannot_hide_guard_failure(self) -> None:
        pass_line = b"PASS: G011 empty-shell guard found no success-without-state shells\n"
        for hidden_fail in (b"G011 empty-shell guard: F\0AIL\n", b"G011 empty-shell guard: F\aAIL\n"):
            with self.subTest(hidden_fail=hidden_fail), tempfile.TemporaryDirectory() as temporary:
                log = Path(temporary) / "guard.log"
                log.write_bytes(pass_line + hidden_fail)
                result = evidence.classify_process(
    ...
    def test_duplicate_marker_is_error(self) -> None:
        self.assertEqual(self.classify("smoke-rv64-duplicate.log")["state"], "error")

    def test_truncated_log_is_error(self) -> None:
        result = self.classify("smoke-rv64-truncated.log")
        self.assertEqual(result["reason_code"], "marker_count")

    def test_panic_after_user_pass_is_error(self) -> None:
        result = self.classify("smoke-rv64-panic-after-pass.log")
    ...
    def test_terminal_sequences_cannot_split_or_hide_runtime_fatals(self) -> None:
        base = (FIXTURES / "smoke-rv64-pass.log").read_bytes()
        suffixes = (
    ...
    def test_incomplete_terminal_control_after_markers_is_error(self) -> None:
        ...
        for suffix in suffixes:
            with self.subTest(suffix=suffix), tempfile.TemporaryDirectory() as temporary:
                ...
                path.write_bytes((FIXTURES / "smoke-rv64-pass.log").read_bytes() + suffix)
                result = evidence.classify_process(
                    case=self.smoke,
                    arch="riscv64",
                    process=process_result(),
    ...
    def test_completed_payload_terminal_controls_cannot_hide_runtime_fatals(self) -> None:
        ...
        for suffix in suffixes:
            with self.subTest(suffix=suffix), tempfile.TemporaryDirectory() as temporary:
                ...
                path.write_bytes(
                    (FIXTURES / "smoke-rv64-pass.log").read_bytes() + suffix
                )
                result = evidence.classify_process(
                    case=self.smoke,
                    arch="riscv64",
    ...
    def test_timeout_wins_over_complete_pass_markers(self) -> None:
        result = self.classify("smoke-rv64-pass.log", timed_out=True)
        self.assertEqual(result["state"], "timeout")

    def test_nonzero_exit_after_markers_is_failure(self) -> None:
        result = self.classify("smoke-rv64-pass.log", exit_code=5)
    ...
    def test_wrong_architecture_is_not_a_pass(self) -> None:
        result = evidence.classify_process(
            ...
            arch="loongarch64",
            process=process_result(),
            raw_log=FIXTURES / "smoke-rv64-pass.log",
        )
        self.assertEqual(result["state"], "error")
    ...
    def test_cleanup_residual_is_never_pass(self) -> None:
        result = self.classify("smoke-rv64-pass.log", residual_processes_killed=True)
    ...
    def test_guard_requires_nonempty_unambiguous_protocol(self) -> None:
        ...
        passed = evidence.classify_process(
            ...
            arch="host",
            process=process_result(),
            raw_log=FIXTURES / "guard-pass.log",
        )
        ambiguous = evidence.classify_process(
            case=case,
            arch="host",
            process=process_result(),
            raw_log=FIXTURES / "guard-ambiguous.log",
        )
...
class ProcessSupervisorTests(unittest.TestCase):
    ...
    @unittest.skipUnless(sys.platform.startswith("linux"), "Linux subreaper test")
