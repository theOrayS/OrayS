#!/usr/bin/env python3
"""Static regression guard for G003 stat/lstat metadata semantics.

The guard is intentionally narrow: it scans the POSIX API VFS surface that G003
        ...
        if default_line := default_stat_line(block):
            findings.append(
                Finding(
                    rel,
                    default_line,
                    "g003-default-stat-success-risk",
        ...
        if (first_matching_line(sys_lstat, API_METADATA_RE) or first_matching_line(sys_lstat, STAT_OPEN_RE)) and not lstat_has_explicit_boundary(sys_lstat):
            ...
            findings.append(
                Finding(
                    ...
                    "g003-lstat-without-nofollow-boundary",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.json:
        ...
        print(f"G003 stat metadata static check: FAIL ({len(findings)} findings)")
        for finding in findings:
            print(finding.display())
    else:
        print("G003 stat metadata static check: PASS (0 findings)")
