#!/usr/bin/env python3
"""Mutation tests for the G014 PR3 workflow policy guard."""
...
import check_g014_pr3_workflow as guard


ROOT = Path(__file__).resolve().parents[1]


class G014Pr3WorkflowGuardTest(unittest.TestCase):
    def copy_tree(self, temporary: str) -> Path:
        ...
        scripts.mkdir()
        for name in ("setup_qemu_pr3.sh", "semantic_evidence_manifest.json"):
    ...
    def test_detects_incomplete_aggregate_shards(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            findings = self.mutate(
                ...
                "build/pr3-shards/la64/missing.json",
            )
            self.assertTrue(any("la64/semantic-evidence" in item for item in findings))
