#!/usr/bin/env python3
"""Static guard for G006 synthetic proc/dev/config honesty boundaries."""
...
def scan_synthetic_fs(root: Path) -> list[str]:
    text = read(root / "examples/shell/src/uspace/synthetic_fs.rs")
    findings: list[str] = []
    lowered = text.lower()
    if "ltp" in lowered or "oskernel2026" in lowered:
        findings.append(
            "examples/shell/src/uspace/synthetic_fs.rs: synthetic proc/config content must not carry LTP/oskernel2026 markers"
        )

    cmdline_block = rust_const_block(text, "SYNTHETIC_PROC_CMDLINE_CONTENT")
    if not cmdline_block:
        findings.append(
            "examples/shell/src/uspace/synthetic_fs.rs: missing SYNTHETIC_PROC_CMDLINE_CONTENT"
        )
    else:
    ...
    if not config_block:
        findings.append(
            "examples/shell/src/uspace/synthetic_fs.rs: missing SYNTHETIC_KERNEL_CONFIG_CONTENT"
        )
    else:
...
def scan_block_devices(root: Path) -> list[str]:
    ...
    if not match:
        findings.append("examples/shell/src/uspace/fd_table.rs: missing synthetic block device capability list")
    else:
        names = re.findall(r'"([^"]+)"', match.group("body"))
        if names != ["vda"]:
            findings.append(
                f"examples/shell/src/uspace/fd_table.rs: synthetic block devices must be exactly ['vda'], got {names!r}"
            )
...
def main() -> int:
    ...
    if findings:
        ...
        for finding in findings:
            print(f"- {finding}")
        return 1
    print("G006 synthetic capability static check: PASS (0 findings)")
    return 0
