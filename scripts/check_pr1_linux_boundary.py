#!/usr/bin/env python3
"""Static guard for the PR1 Linux ABI, user-copy, and syscall boundaries."""
...
def read_required(root: Path, rel: Path, findings: list[str]) -> str:
    path = root / rel
    if not path.is_file():
        findings.append(f"{rel}: required PR1 file is missing")
        return ""
...
def main() -> int:
    ...
    if findings:
        print("PR1 Linux boundary static check: FAIL")
        for finding in findings:
            print(f"- {finding}")
        return 1
    print("PR1 Linux boundary static check: PASS (0 findings)")
    return 0
