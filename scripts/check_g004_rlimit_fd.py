#!/usr/bin/env python3
"""Static regression guard for G004 rlimit/sysconf/fcntl/FD semantics.

The guard is intentionally narrow: it scans only the G004 surfaces called out by
...
def missing_function_finding(rel: Path, name: str) -> Finding:
    return Finding(rel, 1, "g004-missing-function", f"expected Rust function {name} is missing")
    ...
    if not has_honest_fallback and not saw_fallback:
        findings.append(
            Finding(
                rel,
                block.start_line,
                "g004-sysconf-missing-honest-fallback",
                ...
                if ok_idx is not None:
                    findings.append(
                        Finding(
                            ...
                            "g004-rlimit-unsupported-ok-zero",
        ...
        if not has_invalid_resource_errno:
            findings.append(
                Finding(
                    rel,
                    block.start_line,
                    "g004-rlimit-missing-invalid-resource-errno",
        ...
        if not has_null_efault:
            findings.append(
                Finding(
                    rel,
                    block.start_line,
                    "g004-rlimit-missing-efault-boundary",
            ...
            if block and "Ok(0)" in block.text:
                findings.append(
                    Finding(
                        rel,
                        idx,
                        "g004-api-filelike-default-status-ok-zero",
        ...
        if cmd not in block.text:
            findings.append(
                Finding(
                    rel,
                    block.start_line,
                    "g004-api-fcntl-missing-fd-command",
        ...
        if "dup_fd(fd)" in chunk and "FD_CLOEXEC" not in chunk_without_cmd_name:
            findings.append(
                Finding(
                    rel,
                    line_no,
                    "g004-api-dupfd-cloexec-drops-flag",
            ...
            if ok_idx is not None:
                findings.append(
                    Finding(
                        rel,
                        block.start_line + ok_idx,
                        "g004-api-fcntl-stdio-hardcoded-ok-zero",
            ...
            if ok_idx is not None:
                findings.append(
                    Finding(
                        rel,
                        block.start_line + ok_idx,
                        "g004-api-fcntl-unsupported-ok-zero",
        ...
        if MATCH_FALLBACK_OK_ZERO_RE.search(stripped):
            findings.append(
                Finding(
                    rel,
                    line_no,
                    "g004-api-fcntl-fallback-ok-zero",
    ...
    if "resource_is_valid" not in block.text or "LinuxError::EINVAL" not in block.text:
        findings.append(
            Finding(
                rel,
                block.start_line,
                "g004-prlimit-missing-invalid-resource-check",
            ...
            if ok_idx is not None:
                findings.append(
                    Finding(
                        rel,
                        block.start_line + ok_idx,
                        "g004-prlimit-invalid-resource-success",
        ...
        if cmd not in block.text or handler not in block.text:
            findings.append(
                Finding(
                    rel,
                    block.start_line,
                    "g004-shell-fcntl-missing-fd-command",
        ...
        if MATCH_FALLBACK_OK_ZERO_RE.search(stripped):
            findings.append(
                Finding(
                    rel,
                    line_no,
                    "g004-shell-fcntl-fallback-ok-zero",
...
def scan_repo(root: Path = REPO_ROOT) -> list[Finding]:
    ...
    for rel_path, scanner in scanners:
        ...
        if path.exists():
            ...
            findings.append(Finding(rel_path, 1, "g004-missing-file", "expected G004 surface file is missing"))
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.json:
        ...
        print(f"G004 rlimit/sysconf/fcntl/FD static check: FAIL ({len(findings)} findings)")
        for finding in findings:
            print(finding.display())
    else:
        print("G004 rlimit/sysconf/fcntl/FD static check: PASS (0 findings)")
