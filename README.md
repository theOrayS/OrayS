...
- 统一在 `test/` 注册 **16 个静态守卫及 20 个 Python 单元测试套件（492 个方法）**，用于检查
...
python3 -I -S -B -X pycache_prefix=/dev/null test/run_suite.py --profile quick
python3 -I -S -B -X pycache_prefix=/dev/null test/run_suite.py --profile baseline
```

上述 goal-form 命令是兼容入口；runner 启动后会立即切换到隔离模式，但无法撤销脚本
启动前已经执行的 Python startup hook。受控证据运行必须让第一个解释器进程就使用
`-I -S -B -X pycache_prefix=/dev/null`，同时隔离调用者路径、site startup 和被 Git
忽略的本地 bytecode。无论采用哪种写法，都必须以本次生成的 `summary.json` 及其中
`planned == executed == completed` 为结果依据；只有 shell 返回 0 而没有本次汇总并不构成 PASS。

除只读 `--list` 外，canonical `checks`、`unit`、`quick`、`baseline`、
`official` 和 `full` 只允许在干净工作树上执行。runner 在加载仓库内 parser 前先用
封闭 Git 环境检查 tracked/untracked 状态；脏树会直接退出 2，且不会创建输出目录或
`summary.json`，避免把不可归属到单一提交的运行当作证据。

等价的常用 Make 目标为 `make test-list`、`make test-checks`、
`make test-unit`、`make test-quick` 和 `make test-baseline`。通过启动和干净树预检的
执行会在已忽略的 `test/output/` 下生成独立 stdout/stderr 日志和 JSON 汇总；只有所有计划 case 都被
...
`python3 -I -S -B -X pycache_prefix=/dev/null test/run_suite.py --profile official --arch rv|la`，零参数保持
旧入口的 RV 默认值。canonical launcher 只接受架构、可选的
`--output-dir PATH` 和 `--fail-fast`；会在 Python 启动前拒绝 `--manifest`、
`--profile`、重复 `--arch`、`--list` 及其他策略覆盖参数。`official`/`full`
profile 也只能使用 canonical `test/suite_manifest.json`。严格 runner 会执行
manifest 预检、外层超时、进程组控制、
...
```bash
RV_TESTSUITE_IMG=/path/to/sdcard-rv.img python3 -I -S -B -X pycache_prefix=/dev/null test/run_suite.py --profile official --arch rv
LA_TESTSUITE_IMG=/path/to/sdcard-la.img python3 -I -S -B -X pycache_prefix=/dev/null test/run_suite.py --profile official --arch la
```
